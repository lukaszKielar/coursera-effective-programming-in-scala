  ...
  def extractUnsafe: A =
    Try(Await.result(el.value, timeout)) match
      ...
      case Success(Left(errors)) =>
        throw Exception(
          s"Expecting a successful WikiResult but obtained some business error(s): ${errors.mkString("\n")}"
        )
      case Failure(exception) =>
        throw Exception(
          s"Expecting a successful WikiResult but obtained a system failure: ${exception}"
        )

def blockAndCompare[A](
    expected: WikiResult[A],
