...
class WikigraphSuite extends munit.FunSuite:
  ...
  given wikiExceptionGen: Gen[WikiException] =
    Gen.oneOf(
      WikiException.Timeout,
      WikiException.TooManyRequests,
      WikiException.ResourceNotFound("test")
    )

  def wikiResultGen[A](withErrors: Boolean, withFailure: Boolean)(using
  ...
  property("zip without errors") {
    ...
    forAll(genAB) { (a: WikiResult[Int], b: WikiResult[String]) =>
      ...
      val expected = WikiResult(
        a.value.flatMap(eiL =>
          b.value.map(eiR => eiL.flatMap(l => eiR.map(r => l -> r)))
        )
      )
  ...
  property("traverse with domain errors") {
    forAll(
      Gen.containerOf[List, WikiResult[Int]](wikiResultGen[Int](true, false))
    ) { listOfResults =>
      val pure: (Int => String) = x => (x + 42).toString
      ...
      if listOfResults.forall(_.errors.isEmpty) then
        ...
        blockAndCompare(
          WikiResult.successful(expected),
          obtained,
          "Result sequence is incorrect"
        )
      else
        val expectedErrors = listOfResults.flatMap(_.errors)
        val expectedResult = WikiResult(
          Future.successful(Left[Seq[WikiError], Seq[String]](expectedErrors))
        )
        blockAndCompare(
          expectedResult,
          obtained,
  ...
  property("traverse with domain errors and failures") {
    forAll(
      Gen.containerOf[List, WikiResult[Int]](wikiResultGen[Int](true, true))
    ) { listOfResults =>
      val pure: (Int => String) = x => (x + 42).toString
      ...
      if listOfResults.exists(_.failure.nonEmpty) then
        val failure =
          listOfResults.find(_.failure.nonEmpty).flatMap(_.failure).get
    ...
    forAll(wikiResultGen[Int](true, true)) { (w: WikiResult[Int]) =>
      ...
      if w.failure.nonEmpty then
        sameErrorMessage(
          w.failure,
          obtained.failure
        ) :| s"map on a WikiResult failed with a system failure propagates the failure\n - Expected: ${w.failure}\n - Obtained: ${obtained.failure}"
    ...
    forAll(wikiResultGen[Int](true, true)) { (w: WikiResult[Int]) =>
      ...
      if w.failure.nonEmpty then
        sameErrorMessage(
          w.failure,
          obtained.failure
        ) :| s"flatMap on a WikiResult failed with a system failure propagates the failure.\n - Expected: ${w.failure}\n - Obtained: ${obtained.failure} "
      else if w.errors.nonEmpty then
        ...
        blockAndCompare(
          expected,
          obtained,
          "flatMap on successfull WikiResult with a successful function is correct"
        )
    }
  }
  ...
  property("breadthFirstSearch does not fail on errors (2pts)") {
    ...
    val res = Wikigraph(InMemory(g)).breadthFirstSearch(
      ArticleId(1),
      ArticleId(1000),
      4
    )
  ...
  property("distanceMatrix (3pts)") {
    ...
    val exp = WikiResult.successful[Seq[(String, String, Option[Int])]](
      Seq(
        ("TestArticle-1", "TestArticle-3", Option(2)),
    ...
    def setUp(withErrors: Boolean, withFailure: Boolean): Prop = {
      ...
      val failableLinks: Gen[WikiResult[Set[ArticleId]]] =
        val setOfGen = links
          .map(Gen.const)
          .map(g =>
            wikiResultGen(withErrors, withFailure)(using
              summon[Gen[WikiError]],
              g
            )
          )
        Gen
          .sequence[Set[WikiResult[ArticleId]], WikiResult[ArticleId]](setOfGen)
      ...
      val nameSearcher: Gen[ArticleId => WikiResult[String]] =
        ...
        val gens: List[Gen[(ArticleId, WikiResult[String])]] = all.map {
          (node: ArticleId) =>
            wikiResultGen(withErrors, withFailure)(using
              summon[Gen[WikiError]],
        ...
        Gen
          .sequence[List[
            (ArticleId, WikiResult[String])
          ], (ArticleId, WikiResult[String])](gens)
          .map { listOfTuples => (art: ArticleId) =>
        ...
        if links.failure.nonEmpty || links.errors.nonEmpty then
          ...
          if nameFail.nonEmpty then
            ...
            if nameErrors.nonEmpty then
              ...
              (result.extractUnsafe == results.map(
                _.extractUnsafe
