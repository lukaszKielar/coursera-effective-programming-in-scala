...
/** The interface of objects providing access to Wikipedia data.
  *
  * All the methods of this class can fail with a system failure of type
  * [[wikigraph.errors.WikiException]]
  */
trait Wikipedia:
  /** Retrieves the ids of the pages linked on a page.
    ...
    */
  def linksFrom(art: ArticleId)(using
      ExecutionContext
  ): WikiResult[Set[ArticleId]]

  /** Retrieves the title of the page corresponding to the provided id.
    *
    * If the provided id does not identify an article in the data or if page
    * does not have a title, the computation fails with a
